...
let diffs path c_ptr_l c_ptr_r =
    ...
    try
        ...
        let ptr_arr = Ctypes.CArray.make (ptr void) ~initial:ptr_add 3 in
        let ptr_arr = Ctypes.CArray.set ptr_arr 1 ptr_del in
        let ptr_arr = Ctypes.CArray.set ptr_arr 2 ptr_inter in
        let ptr_start = Ctypes.CArray.start ptr_arr in
        let ret_ptr = Ctypes.Root.create ptr_start
    with
